...
max_radius     = [1500; 0]; % maximum radius of clusters in all chanels respectively (should be the same size as channel_token)
min_nr_locs    = [300; 0];  % minimum nr of locs within min_radius

P              = [200; 0]; % blobs with fewer pixels will be removed from mask
...
remove_border  = 1; % to remove the edges of images if there are edge artefacts
...
%     
    ...
    for nch = 1:N_channel
        if max_radius(nch)~= 0 && min_nr_locs(nch) ~= 0
            ...
            filename{nch} = char(strcat(area_token,['_reconstruction_' num2str(nch) 'channel_density_filtered.tif']));
            if ~essence; imwrite(flip(flip(img(nch).channel,2),1), fullfile(path_output, filename{nch})); end
